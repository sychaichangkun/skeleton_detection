...
public:
      ...
      it_(nh_)
  {
    ...
    {
      ROS_FATAL("Couldn't create user tracker. Cann't initialize NiTE");
      ros::shutdown();
      return;
    }
...
/////////////////////////////////////////////////////////////////////////////////////////////////////
        ...
        {
            userConfidence+=it->second.getPositionConfidence();
            publishJointTF(it->first, it->second, user.getId());
        }
        if(userConfidence>maxUserConfidence)
        {
          maxUserConfidence=userConfidence;
          maxUserID=i;
          for (JointMap::iterator it=named_j.begin();it!=named_j.end();it++)
          {
            if (it->first=="head")
             {
              tf::StampedTransform sk2map;
              try{
                std::stringstream frame_id_stream;
                std::string frame_id;
                frame_id_stream << depth_frame << "/user_" << user.getId() << "/" << it->first;
                frame_id = frame_id_stream.str();
        ...
        //cck add
        //get the max confidence among users and publish his joint TF
//        float jointConfidence=0.0;
//        JointMap::iterator it2=named_j.begin();
//        for (JointMap::iterator it = named_j.begin(); it != named_j.end(); ++it)
//        {
//          if(it->second.getPositionConfidence()>jointConfidence)
//          {
//              jointConfidence=it->second.getPositionConfidence();
//              it2=it;
//          }
//          //////
//        }
//        publishJointTF(it2->first,it2->second,user.getId());
